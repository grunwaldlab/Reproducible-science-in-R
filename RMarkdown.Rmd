...
Types of uses range from adding minor bits of code to time-stamp notes to running complex analyses and making a report of the results.
...
Note that the rendering of RMarkdown is implemented using an R package called `knitr` so you might see references to things like "knitr options".
This is also the reason the button you press to render RMarkdown into its output document is labeled "Knit HTML".


### Mixing R and Markdown

There are two ways to add R code to Markdown: inline, and chunks.
The syntax to adding inline code is the same as specifying a monospace font (surrounding text with <code>\`</code>), except that an `r` is added after the first <code>\`</code>.
...
Unlike the inline method, chunks of code defined this way have many options controlling how they are handled; we will cover some of these options soon.
...
\`\`\`{r}
var( c(1, 3, 2.4, 6, 7.2) ) # how to calculate standard deviation in R
\`\`\`
...
There two primary ways graphs are made in R: the original included graphics capabilities and the newer ggplot2 package. 
...
However, this package is focused mostly on making table for PDF output rather than HTML. 
...
This header information of RMarkdown file in a language called YAML, used to store information.
...
```{r yaml, results='asis', echo = FALSE}
rmd_example(height = 300, prefix = TRUE, "
---
output:
  html_document:
    toc: true
...
There are a few ways of adding references/bibliographies to your RMarkdown document automatically.
We will be using an external file containing a list of references in a standard format called BibTeX. 
Like RMarkdown, this is simply a plain text format viewable in any text editor.
Text files of any format can be made in RStudio by clicking on the "New file" dropdown menu and choosing "Text file".

**`r step()` Click on the "New file" dropdown menu and choose "Text file". Paste the text below into the file and save it as "bibliography.bibtex".**
...
```{r print_bib, results='hide', echo = FALSE, }
paste0("```\n",
       bib_content,
       "```\n")
```
...
Next, we will associate the bibliography file with the RMarkdown file.
This is done using a setting in the YAML header.

**`r step()` Replace the YAML header _at the start_ of your Markdown document with the one below.**
...
Citing any papers will cause the bibliography to be added to the bottom of the rendered document. 
